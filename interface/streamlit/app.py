...
def run_simulations(
    ...
    val_a_audited_strict, risk_a_strict, date_a_strict, logs_a_strict = backtest(
        "ModelA_Signal", strict_dd
    )
    ...
    val_b_audited_strict, risk_b_strict, date_b_strict, logs_b_strict = backtest(
        "ModelB_Signal", strict_dd
    )
    ...
    return {
        ...
        "logs": {
            "ModelA": {
                "std": logs_a_std,
                "strict": logs_a_strict,
                "halted_std": risk_a_std,
                "halted_strict": risk_a_strict,
                "date_std": date_a_std,
                "date_strict": date_a_strict,
            },
            "ModelB": {
                "std": logs_b_std,
                "strict": logs_b_strict,
                "halted_std": risk_b_std,
                "halted_strict": risk_b_strict,
                "date_std": date_b_std,
                "date_strict": date_b_strict,
            },
        },
...
def get_metrics_table(sim_results, mode="strict"):
    ...
    def compute_stats(series, name):
        ...
        info_ratio = (
            np.sqrt(252) * active_returns.mean() / tracking_error if tracking_error > 0 else 0.0
        )
...
mode_label = (
    "🛠️ Manual Box-by-Box Mode"
    if st.session_state.execution_mode == "manual"
    else "🤖 Autonomous Multi-Agent Mode"
)
mode_color = "#FFD600" if st.session_state.execution_mode == "manual" else "#66FCF1"
mode_desc = (
    "Scroll down to 'OpenLogic Finance 6-Box Architectural Comparison' and click '▶️ Execute Data Ingestion & indicator Prep' in Box 1 to begin."
    if st.session_state.execution_mode == "manual"
    else "Click the 'Run Autonomous Agent Pipeline' button below to orchestrate the synchronized multi-agent loop."
)
...
with btn_col2:
    if st.button("🛠️ Enter Manual Box-by-Box Mode", width="stretch"):
        ...
        st.session_state.manual_boxes_run = {
            1: False,
            2: False,
            3: False,
            4: False,
...
if st.session_state.execution_mode == "autonomous" and not st.session_state.pipeline_run:
    ...
    stages = [
        (
            "📦 Box 1: Data Ingestion & Timezone Localization",
            0.0,
            [
                "[Market Data Agent] 📡 Pinging historical database for ticker: "
                + asset_ticker
                + "...",
        ...
        (
            "🔬 Box 2: Feature Engineering & Pre-Trained Weights Projection",
            0.25,
            [
                "[Feature Eng Agent] 🧪 Calculating indicators: Fast SMA("
                + str(fast_sma_p)
                + "), Slow SMA("
                + str(slow_sma_p)
                + "), RSI("
        ...
        (
            "🧪 Box 3: Simulated LEAN Engine Strategy Testing",
            0.50,
            [
        ...
        (
            "🛡️ Box 4: Active Drawdown Risk Auditing",
            0.75,
            [
        ...
        (
            "⚡ Box 5: Live API Configuration & Paper Execution",
            0.90,
            [
        ...
        (
            "📈 Box 6: System Orchestration Summary",
            1.0,
            [
    ...
    for stage_name, progress_val, stage_logs in stages:
        ...
        if "Box 2" in stage_name:
            current_logs.append(
                "[Model Engine Agent] 🔬 Running Walk-Forward dynamic training on historical dataset..."
            )
            ...
            stage_logs.append(
                f"[Model Engine Agent] 📊 Dynamically trained Walk-Forward weights! OS Accuracy: {train_m.get('accuracy', 0.0):.4f}"
            )
            stage_logs.append(
                f"[Model Engine Agent] 📊 OS ROC AUC: {train_m.get('auc', 0.0):.4f} | Precision: {train_m.get('precision', 0.0):.4f} | F1: {train_m.get('f1', 0.0):.4f}"
            )

        # In Box 3, actually run LEAN backtests live via LeanEngineBridge!
        if "Box 3" in stage_name:
            current_logs.append(
                "[Backtest Agent] 🚀 INITIALIZING LIVE QUANTCONNECT LEAN CLOUD BACKTEST..."
            )
            current_logs.append(
                f"[Backtest Agent] 📡 Command: lean cloud push --project {asset_ticker}"
            )
            ...
            try:
                ...
                bridge_a = LeanEngineBridge(
                    project_path="strategy_testing/lean_engine/logistic_regression_project"
                )
                check_inst = bridge_a.check_lean_installed()
                if not check_inst["installed"]:
                    current_logs.append(
                        "[Backtest Agent] ⚠️ LEAN CLI not installed locally. Falling back to local backtest simulator."
                    )
                else:
                    current_logs.append(
                        "[Backtest Agent] 📡 Executing Model A Cloud Backtest... (please wait)"
                    )
                    terminal_html = f"""
                    <div class="terminal-console">
                        {"<br>".join([f"<span style='color: #66FCF1;'>></span> {l}" for l in current_logs[-12:]])}
                    ...
                    if res_a.success:
                        current_logs.append(
                            f"[Backtest Agent] 🎉 Model A LEAN Cloud Success! Net Profit: {res_a.total_return_pct}% | Orders: {res_a.total_orders}"
                        )
                        st.session_state.lean_res_a = res_a
                    else:
                        current_logs.append(
                            f"[Backtest Agent] ❌ Model A Cloud Backtest failed: {res_a.stderr[:100]}"
                        )

                # 2. Run Model B live Cloud backtest
                bridge_b = LeanEngineBridge(
                    project_path="strategy_testing/lean_engine/sma_crossover_project"
                )
                if check_inst["installed"]:
                    current_logs.append(
                        "[Backtest Agent] 📡 Executing Model B Cloud Backtest... (please wait)"
                    )
                    terminal_html = f"""
                    <div class="terminal-console">
                        {"<br>".join([f"<span style='color: #66FCF1;'>></span> {l}" for l in current_logs[-12:]])}
                    ...
                    if res_b.success:
                        current_logs.append(
                            f"[Backtest Agent] 🎉 Model B LEAN Cloud Success! Net Profit: {res_b.total_return_pct}% | Orders: {res_b.total_orders}"
                        )
                        st.session_state.lean_res_b = res_b
                    else:
                        current_logs.append(
                            f"[Backtest Agent] ❌ Model B Cloud Backtest failed: {res_b.stderr[:100]}"
                        )
            except Exception as ex:
                current_logs.append(
                    f"[Backtest Agent] ⚠️ LEAN CLI connection error: {ex}. Using local high-fidelity backtest metrics."
                )
    ...
    st.success(
        "⚡ Autonomous Agentic Execution Completed Successfully! Scroll down to inspect the synchronized OpenLogic Finance 6-Box panels."
    )
    st.balloons()
...
with tabs[0]:
    ...
    if st.session_state.execution_mode == "manual" and not st.session_state.manual_boxes_run.get(
        1, False
    ):
        st.markdown(
            """
        ...
        if st.button("▶️ Execute Data Ingestion & indicator Prep", key="run_box_1", width="stretch"):
            with st.spinner(
                "Connecting to ingestion gateway, scrubbing timezone-aware index, and aligning indicators..."
            ):
                time.sleep(1.0)
        ...
        with col1:
            ...
            if ExplanationEngine is not None:
                ...
                exp_lvl = st.radio(
                    ...
                    ["Beginner Friendly (Teddy Bear)", "Academic Quantitative (Jim Simons)"],
                    horizontal=True,
                )
        ...
        with col2:
            ...
            fig1.add_trace(
                go.Scatter(
                    ...
                    y=sim_df["Close"],
                    name="Close Price",
                    line=dict(color="#8F94FB", width=1.5),
                ),
                row=1,
                col=1,
            )
            fig1.add_trace(
                go.Scatter(
                    x=sim_df.index,
                    y=sim_df["Fast_SMA"],
                    name=f"Fast SMA ({fast_sma_p})",
                    line=dict(color="#66FCF1", width=1.2, dash="dot"),
                ),
                row=1,
                col=1,
            )
    ...
    if not st.session_state.manual_boxes_run.get(1, False):
        ...
        with pool_cols[0]:
            sel_a = st.selectbox(
                ...
                model_pool,
                index=model_pool.index(st.session_state.model_a_selected)
                if st.session_state.model_a_selected in model_pool
                else 0,
                key="sel_model_a",
            )
        with pool_cols[1]:
            sel_b = st.selectbox(
                ...
                model_pool,
                index=model_pool.index(st.session_state.model_b_selected)
                if st.session_state.model_b_selected in model_pool
                else 1,
                key="sel_model_b",
            )
        ...
        if sel_a != st.session_state.model_a_selected or sel_b != st.session_state.model_b_selected:
            ...
            st.session_state.lean_res_b = None
            st.session_state.pipeline_run = False
            st.session_state.manual_boxes_run[2] = (
                False  # re-lock Box 2 itself when selection changes!
            )
            st.session_state.manual_boxes_run[3] = False
            st.session_state.manual_boxes_run[4] = False
            st.session_state.manual_boxes_run[5] = False
            st.session_state.manual_boxes_run[6] = False
            st.toast(
                "🔄 Model selection updated! Execute Box 2 to build configurations.", icon="🔄"
            )
            safe_rerun()
        ...
        if (
            st.session_state.execution_mode == "manual"
            and not st.session_state.manual_boxes_run.get(2, False)
        ):
            st.markdown(
                """
            <div class="obsidian-card" style="border-left: 4px solid #8F94FB; margin-bottom: 25px;">
            ...
            if st.button(
                "▶️ Execute Model Library & Signal Logic", key="run_box_2", width="stretch"
            ):
                with st.spinner(
                    "Engineering feature indicators, instantiating Model A & Model B, and generating signal ledger..."
                ):
                    time.sleep(1.0)
            ...
            def render_dynamic_model_card(model_name: str, card_color: str, is_model_a: bool):
                if model_name == "Logistic Regression Strategy":
                    ...
                    with st.expander("🛠️ Show Feature Engineering Formulations", expanded=True):
                        st.latex(
                            r"\text{SMA Ratio} = \frac{\text{Fast SMA}}{\text{Slow SMA}} - 1.0"
                        )
                        st.latex(r"\text{RSI Normalized} = \frac{\text{RSI} - 50.0}{50.0}")
                        st.latex(r"\text{Momentum} = \frac{\text{Close}}{\text{Prev Close}} - 1.0")

                    weights = sim_results.get(
                        "weights", {"sma_ratio": -0.1710, "rsi_norm": -0.0510, "momentum": -0.0290}
                    )
                    feature_stds = sim_results.get(
                    ...
                    styled_df = map_func(
                        lambda x: (
                            "background-color: rgba(0, 230, 118, 0.15); color: #00E676; font-weight: bold;"
                            if x in ["GOLDEN_CROSS", "BUY"]
                            else (
                                "background-color: rgba(255, 61, 0, 0.15); color: #FF3D00; font-weight: bold;"
                                if x in ["DEATH_CROSS", "SELL"]
                                else ""
                            )
                        )
                    )
                    ...
                    with st.expander(
                        "🔬 View Model Specifications & Roadmapped Hyperparameters", expanded=True
                    ):
            ...
            with col_a:
                ...
                render_dynamic_model_card(
                    st.session_state.model_a_selected, "#8F94FB", is_model_a=True
                )
            with col_b:
                st.markdown("### 🚗 Model B Details")
                render_dynamic_model_card(
                    st.session_state.model_b_selected, "#FFD600", is_model_a=False
                )
...
with tabs[2]:
    ...
    if st.session_state.execution_mode == "manual" and not st.session_state.manual_boxes_run.get(
        ...
        with col_lc1:
            ...
            st.code(
                "Model A: strategy_testing/lean_engine/logistic_regression_project\nModel B: strategy_testing/lean_engine/sma_crossover_project",
                language="text",
            )
        ...
        if not is_a_active or not is_b_active:
            ...
            if st.button(
                ...
                if not check_installed["installed"]:
                    st.error(
                        f"LEAN CLI check failed: {check_installed['version']}. Please run `pip install lean && lean login`."
                    )
                else:
                    # 1. Run Model A
                    status_a = st.status(
                        "Executing Model A (Logistic Regression) Live LEAN Cloud Backtest...",
                        expanded=True,
                    )
                    with status_a:
                        ...
                        try:
                            ...
                            if res_a.success:
                                status_a.update(
                                    label="✅ Model A Cloud Backtest Completed Successfully!",
                                    state="complete",
                                )
                            else:
                                status_a.update(
                                    label="❌ Model A Cloud Backtest Failed!", state="error"
                                )
                    ...
                    # 2. Run Model B
                    status_b = st.status(
                        "Executing Model B (SMA Crossover) Live LEAN Cloud Backtest...",
                        expanded=True,
                    )
                    with status_b:
                        ...
                        try:
                            ...
                            if res_b.success:
                                status_b.update(
                                    label="✅ Model B Cloud Backtest Completed Successfully!",
                                    state="complete",
                                )
                            else:
                                status_b.update(
                                    label="❌ Model B Cloud Backtest Failed!", state="error"
                                )
        ...
        if st.session_state.lean_res_a is not None or st.session_state.lean_res_b is not None:
            ...
            metrics_a = (
                extract_qc_metrics(st.session_state.lean_res_a)
                if st.session_state.lean_res_a
                else {}
            )
            metrics_b = (
                extract_qc_metrics(st.session_state.lean_res_b)
            ...
            st.markdown(
                "Below are the local historical backtest simulation metrics (daily close-to-close) for all 3 strategies:"
            )
            ...
            st.markdown(
                "Below is the high-fidelity cloud execution metrics comparison for both strategies running live on the QuantConnect LEAN Engine:"
            )
            ...
            if res_a is not None or res_b is not None:
                ...
                if res_a is not None and res_a.success and res_a.total_return_pct is not None:
                    ...
                    scale_a = (
                        (target_final_a - 100000.0) / (final_local_a - 100000.0)
                        if (final_local_a - 100000.0) != 0
                        else 1.0
                    )
                ...
                if res_b is not None and res_b.success and res_b.total_return_pct is not None:
                    ...
                    scale_b = (
                        (target_final_b - 100000.0) / (final_local_b - 100000.0)
                        if (final_local_b - 100000.0) != 0
                        else 1.0
                    )
            ...
            fig_eq = go.Figure()
            fig_eq.add_trace(
                go.Scatter(x=sim_df.index, y=y_a, name=label_a, line=dict(color="#8F94FB", width=2))
            )
            fig_eq.add_trace(
                go.Scatter(x=sim_df.index, y=y_b, name=label_b, line=dict(color="#00E676", width=2))
            )
            fig_eq.add_trace(
                go.Scatter(
                    x=sim_df.index,
                ...
                if st.session_state.lean_res_a:
                    ...
                    if res_a.success:
                        ...
                        if not df_stats_a.empty:
                            ...
                            st.text_area(
                                f"{st.session_state.model_a_selected} Output",
                                res_a.full_summary or res_a.stdout,
                                height=300,
                            )
                    else:
                    ...
                    st.info(
                        f"{st.session_state.model_a_selected} backtest has not been executed yet."
                    )
                ...
                if st.session_state.lean_res_b:
                    ...
                    if res_b.success:
                        ...
                        if not df_stats_b.empty:
                            ...
                            st.text_area(
                                f"{st.session_state.model_b_selected} Output",
                                res_b.full_summary or res_b.stdout,
                                height=300,
                            )
                    else:
                    ...
                    st.info(
                        f"{st.session_state.model_b_selected} backtest has not been executed yet."
                    )
    ...
    if st.session_state.execution_mode == "manual" and not st.session_state.manual_boxes_run.get(
        ...
        if not st.session_state.manual_boxes_run.get(3, False):
            ...
            if st.button("▶️ Run Active Drawdown Risk Audit", key="run_box_4", width="stretch"):
                with st.spinner(
                    "Reviewing peak-to-trough historical drawdowns and checking risk boundary constraints..."
                ):
                    time.sleep(1.0)
        ...
        with col1:
            ...
            custom_dd = st.slider(
                "Configure Risk Auditor Drawdown Limit (Veto Threshold)",
                0.05,
                0.20,
                0.08,
                step=0.01,
        ...
        with col2:
            ...
            console_logs = []
            console_logs.append(
                f"[System Init] Active Risk Auditor bound to {asset_ticker} daily feed..."
            )
            console_logs.append(
                f"[System Config] Veto Threshold established at: {custom_dd * 100:.1f}% max drawdown."
            )

            if len(a_logs) > 0:
                console_logs.append(
                    "[RISK DETECTED] Model A (Logistic Regression) experienced active drawdown violation."
                )
                for log in a_logs:
                    console_logs.append(
                        f"🔴 Model A Breach Date: {log['date'].strftime('%Y-%m-%d')} | Value: ${log['val']:,.2f} | {log['msg']}"
                    )
            else:
            ...
            if len(b_logs) > 0:
                console_logs.append(
                    "[RISK DETECTED] Model B (SMA Crossover) experienced active drawdown violation."
                )
                for log in b_logs:
                    console_logs.append(
                        f"🔴 Model B Breach Date: {log['date'].strftime('%Y-%m-%d')} | Value: ${log['val']:,.2f} | {log['msg']}"
                    )
            else:
    ...
    if st.session_state.execution_mode == "manual" and not st.session_state.manual_boxes_run.get(
        ...
        if not st.session_state.manual_boxes_run.get(4, False):
            ...
            if st.button("▶️ Initialize Paper & Live Gateways", key="run_box_5", width="stretch"):
                with st.spinner(
                    "Establishing secure broker API links, configuring order tickets, and verifying routing paths..."
                ):
                    time.sleep(1.0)
        ...
        with col2:
            ...
            st.markdown(
                "Enter simulated orders to test endpoints and slippage routing dynamically."
            )
            ...
            if st.button("Generate Paper Order Ticket"):
                ...
                st.session_state.order_tickets.append(
                    f"[{current_time}] ORDER RECEIVED: {t_side} {t_shares} shares of {t_ticker}"
                )
                st.session_state.order_tickets.append(
                    f"[{current_time}] ORDER ROUTING: ARCA SMART ROUTER (Priority: Speed)"
                )
                st.session_state.order_tickets.append(
                    f"[{current_time}] ORDER FILLED: {t_shares} shares of {t_ticker} (Avg Slippage: +{est_slippage:.4f}%)"
                )
                st.session_state.order_tickets.append(
                    f"[{current_time}] Broker ledger successfully updated."
                )

            terminal_logs = (
                st.session_state.order_tickets
                if len(st.session_state.order_tickets) > 0
                else ["[System Idle] Waiting for paper execution order ticket triggers..."]
            )
...
with tabs[5]:
    ...
    if st.session_state.execution_mode == "manual" and not st.session_state.manual_boxes_run.get(
        ...
        if not st.session_state.manual_boxes_run.get(5, False):
            ...
            if st.button(
                "▶️ Verify Multi-Agent Orchestration & Telemetry", key="run_box_6", width="stretch"
            ):
                with st.spinner(
                    "Assembling agentic trace history and calculating confidence drift profiles..."
                ):
                    time.sleep(1.0)
